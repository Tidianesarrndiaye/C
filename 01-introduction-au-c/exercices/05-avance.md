...
19. **Tableau dynamique.** L'utilisateur choisit la taille ; remplis, affiche, libère. Vérifie
    qu'il n'y a **aucune fuite** — Valgrind sous Linux/WSL, Dr. Memory sous Windows.
20. **`strdup` maison.**
...
23. **Fuite volontaire.** Introduis une fuite, retrouve-la avec ton détecteur de fuites,
    corrige-la.
24. **Liste chaînée.** Insertion en tête et en queue, suppression, recherche, affichage,
    libération complète. Zéro fuite au détecteur.
...
28. **Sanitizer.** Fais tourner un de tes programmes avec `-fsanitize=address,undefined`
    (Linux/WSL ; indisponible sous MinGW-w64 — voir chapitre 30).
